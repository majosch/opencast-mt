...
opencast_jdbc_trytoconnect() {
  ...
  driver=$(awk -F "=" '/org\.opencastproject\.db\.jdbc\.driver/ {print $2}' ${OPENCAST_CONFIG}/custom.properties | tr -d ' ')
  url=$(awk -F "=" '/org\.opencastproject\.db\.jdbc\.url/ {print $2}' ${OPENCAST_CONFIG}/custom.properties | tr -d ' ')
  user=$(awk -F "=" '/org\.opencastproject\.db\.jdbc\.user/ {print $2}' ${OPENCAST_CONFIG}/custom.properties | tr -d ' ')
  password=$(awk -F "=" '/org\.opencastproject\.db\.jdbc\.pass/ {print $2}' ${OPENCAST_CONFIG}/custom.properties | tr -d ' ')
